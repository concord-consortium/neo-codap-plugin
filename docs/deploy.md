...
`models-resources/neo-codap-plugin/` (e.g. `branch/<name>/...`) by the `s3-deploy` job in
[`ci.yml`](../.github/workflows/ci.yml). The Playwright test report is uploaded to
`models-resources/neo-codap-plugin/playwright-report/...` by
...
`s3://models-resources/neo-codap-plugin/`.
